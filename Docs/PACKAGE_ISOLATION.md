...
- Windows/Android Client：Cook `/Game/UI`、`/Game/Art/Mahjong` 和目标中性的联网共享
  `MahjongRoomMap`。客户端进入后本地生成 `MahjongRoomPresentationActor`。
- LinuxServer/WindowsServer：只 Cook 无渲染、无目标专属类依赖的 `MahjongRoomMap`，并将 `/Game/UI`、
  `/Game/Art` 加入 NeverCook。
- `MahjongRoomVisualPreviewMap` 仅供编辑器人工调整桌面、摄像机和灯光，不进入 Shipping Cook。
