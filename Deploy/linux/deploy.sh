...
validate_environment() {
  ...
  if [[ "$variant" == unreal && "$game_server_map" != "/Game/Maps/MahjongServerMap" ]]; then
    echo "GAME_SERVER_MAP must be /Game/Maps/MahjongServerMap for the Unreal server variant." >&2
    exit 1
  fi
